...
<img src="https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450324.jpeg" alt="DOM树构建过程" style="zoom: 80%;" />
...
![CSSOM树](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261402867.jpeg)

如果想要更详细地去体验一下关键渲染路径的构建，可以使用`Chrome`开发者工具中的`Timeline`功能，它记录了浏览器从请求页面资源一直到渲染的各种操作过程，甚至还可以录制某一时间段的过程（建议不要去看太大的网站，信息会比较杂乱）。

![Timeline](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261403429.png)
...
![渲染树](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450088.png)
...
![盒子模型](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261405051.png)
...
![上述代码的布局结果](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450037.png)
...
> 1. 等待所有CSS下载并解析完——阻塞渲染
>    CSSOM是完备的，这种完备是指所有的CSS（内联、内部和外部）都已经下载完，并解析完，只有CSSOM和DOM的解析完全结束，浏览器才会进入下一步的渲染。
> 2. 中途Script脚本——阻塞解析
>    当浏览器遇到一个 script 标签时，DOM 构建将暂停，直至脚本完成执行，然后继续构建DOM。
>    如果JS脚本还操作了CSSOM，而正好这个CSSOM还没有下载和构建，浏览器甚至会延迟脚本执行和构建DOM，直至完成其CSSOM的下载和构建 ，因此每次去执行JavaScript脚本都会严重地阻塞DOM树的构建
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450332.png)
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450453.png)
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450136.png)
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450602.png)
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202210261450736.png)
