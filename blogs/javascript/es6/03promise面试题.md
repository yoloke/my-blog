...
- 因此`promise.then`并不会执行，它**只有在被改变了状态之后**才会执行。
...
请仔细看看，`fn`函数它是直接返回了一个`new Promise`的，而且`fn`函数的调用是在`start`之前，所以它里面的内容应该会先执行。
...
**注意⚠️**：之前我们很容易就以为看到`new Promise()`就执行它的第一个参数函数了，其实这是不对的，就像这两道题中，我们得注意它是不是被**包裹在函数**当中，如果是的话，只有在函数调用的时候才会执行。
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202211061819942.webp)
...
上面这道题搞懂了之后，就可以来做做这道了，应该能很快就给出答案：
...
`.race()`的作用也是接收一组异步任务，然后并行执行异步任务，只保留取**第一个执行完成**的异步操作的结果，其他的方法仍在执行，不过执行结果会被抛弃。
...
![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202211070928673.webp)

为了方便你查看，我截了个图，不过代码在后面也有

(说真的，要我看这一大长串代码我也不愿意...)

![img](https://could-img.oss-cn-hangzhou.aliyuncs.com/202211070928729.webp)
